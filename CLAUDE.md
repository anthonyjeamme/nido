...
5. **Migrations SQL uniquement via fichiers versionnés** dans `supabase/migrations/` —
   ...
   Chaque migration qui crée une table déclare aussi ses privilèges : **REVOKE ALL
   puis GRANT explicite** (anon / authenticated / service_role). Le revoke d'abord
   est indispensable : le CLI local (ancien) accorde des privilèges par défaut que
   le cloud et la CI n'accordent plus — sans revoke, les tests RLS divergent.
